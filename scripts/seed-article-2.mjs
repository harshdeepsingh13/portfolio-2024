...
// ---------------------------------------------------------------------------
// Word count (strips HTML tags, counts words)
// ---------------------------------------------------------------------------
const strippedText = body_html.replace(/<[^>]*>/g, " ").replace(/\s+/g, " ").trim();
const wordCount = strippedText.split(" ").filter((w) => w.length > 0).length;
...
if (existing) {
  console.log(`Post with slug "${slug}" already exists — updating content...`);
  // Preserve the original publishedAt so re-runs don't change the post date
  const { publishedAt: _ignored, ...updateFields } = post;
  await BlogPost.findOneAndUpdate({ slug }, updateFields, { new: true });
...
console.log(`Published at: ${post.publishedAt.toISOString()}`);
